...
namespace VocaNova.API.Infrastructure.Caching.Auth;

public sealed class RedisUserProfileCache : IUserProfileCache
{
    private static readonly TimeSpan ProfileTtl = TimeSpan.FromMinutes(5);
    private static readonly JsonSerializerOptions JsonOptions = new(JsonSerializerDefaults.Web);

    private readonly RedisSettings _settings;
    private readonly ILogger<RedisUserProfileCache> _logger;
    private readonly Lazy<Task<IConnectionMultiplexer?>> _connection;

    public RedisUserProfileCache(
        IOptions<RedisSettings> settings,
        ILogger<RedisUserProfileCache> logger)
    {
    ...
    public async Task<UserProfile?> GetAsync(uint userId, CancellationToken cancellationToken = default)
    {
        ...
        return cached.HasValue
            ? JsonSerializer.Deserialize<UserProfile>(cached!, JsonOptions)
            : null;
    }

    public async Task SetAsync(UserProfile profile, CancellationToken cancellationToken = default)
    {
        var database = await GetDatabaseAsync();
        if (database is null)
        {
        ...
        await database.StringSetAsync(GetKey(profile.UserId), payload, ProfileTtl);
    }
    ...
    private async Task<IConnectionMultiplexer?> ConnectAsync()
    {
        ...
        {
            _logger.LogWarning(exception, "Redis is unavailable. User profile cache is disabled.");
            return null;
        }
    }
