...
public sealed class RedisUserProfileCache : IUserProfileCache
{
    ...
    public async Task<UserProfileDto?> GetAsync(uint userId, CancellationToken cancellationToken = default)
    {
        ...
        return cached.HasValue
            ? JsonSerializer.Deserialize<UserProfileDto>(cached!, JsonOptions)
            : null;
    }

    public async Task SetAsync(UserProfileDto profile, CancellationToken cancellationToken = default)
    {
        var database = await GetDatabaseAsync();
        if (database is null)
        {
