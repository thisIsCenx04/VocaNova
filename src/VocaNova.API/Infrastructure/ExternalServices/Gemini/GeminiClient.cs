...
public sealed class GeminiClient : IGeminiClient
{
    ...
    private readonly AiGradingConfiguration _configuredSettings;
    private readonly IAiGradingConfigurationService? _configService;
    private readonly ILogger<GeminiClient> _logger;

    public GeminiClient(
        HttpClient httpClient,
        IOptions<AiGradingConfiguration> settings,
        ILogger<GeminiClient> logger,
        IAiGradingConfigurationService? configService = null)
    {
    ...
    /// </summary>
    private static Uri BuildRequestUri(string model, AiGradingConfiguration settings)
    {
        var endpoint = string.IsNullOrWhiteSpace(settings.Endpoint)
            ? new AiGradingConfiguration().Endpoint
            : settings.Endpoint.Trim().TrimEnd('/');

        return new Uri($"{endpoint}/{NormalizeModelName(model)}:generateContent");
    }

    private static IReadOnlyList<string> GetModelCandidates(AiGradingConfiguration settings)
    {
