...
public sealed class KnnLearningRepository : IKnnLearningRepository
{
    ...
    public async Task<IReadOnlyCollection<KnnTopicAnswerStatsDto>> GetTopicAnswerStatsAsync(
        uint userId,
        CancellationToken cancellationToken = default)
    {
        ...
        return rows
            .Select(row => new KnnTopicAnswerStatsDto(
                row.TopicId,
                row.CorrectCount,
                row.TotalCount))
            .ToArray();
    ...
    public async Task<IReadOnlyCollection<KnnMasteredWordDto>> GetMasteredWordsAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        if (userIds.Count == 0)
        {
            return Array.Empty<KnnMasteredWordDto>();
        }
        ...
        return await _dbContext.UserWordProgresses
            ...
            .Select(progress => new KnnMasteredWordDto(progress.UserId, progress.WordId))
            .ToListAsync(cancellationToken);
    }
    ...
    public async Task<IReadOnlyCollection<WordRecommendationDto>> GetWordRecommendationDtosAsync(
        IReadOnlyDictionary<uint, double> scoresByWordId,
        int limit,
        CancellationToken cancellationToken = default)
    {
        ...
        return rows
            .Select(row => new WordRecommendationDto(
                row.WordId,
                row.Word,
                row.PhoneticUk,
                row.PrimaryMeaning,
