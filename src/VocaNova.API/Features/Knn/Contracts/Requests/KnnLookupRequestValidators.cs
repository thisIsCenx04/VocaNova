...
public sealed class CreateRegionRequestValidator : AbstractValidator<CreateRegionRequest>
{
    ...
    {
        Include(new RegionRules<CreateRegionRequest>(request => request.Name, request => request.Code));
    }
}
...
public sealed class UpdateRegionRequestValidator : AbstractValidator<UpdateRegionRequest>
{
    ...
    {
        Include(new RegionRules<UpdateRegionRequest>(request => request.Name, request => request.Code));
    }
}
...
internal sealed class AgeRangeRules<T> : AbstractValidator<T>
{
        ...
        Expression<Func<T, int>> displayOrder)
    {
        ...
        RuleFor(name).NotEmpty().MaximumLength(50);
        RuleFor(minAge).GreaterThanOrEqualTo(0).When(request => minAgeFunc(request).HasValue);
        RuleFor(maxAge).GreaterThanOrEqualTo(0).When(request => maxAgeFunc(request).HasValue);
...
internal sealed class RegionRules<T> : AbstractValidator<T>
{
    ...
    {
        RuleFor(name).NotEmpty().MaximumLength(100);
        RuleFor(code)
            .NotEmpty()
            .MaximumLength(10)
            .Matches("^[A-Za-z0-9_-]+$")
...
internal sealed class NamedDescriptionRules<T> : AbstractValidator<T>
{
    ...
    {
        RuleFor(name).NotEmpty().MaximumLength(100);
        RuleFor(description).MaximumLength(255);
    }
}
