...
public sealed class PersonalTopicMutationRepository : IPersonalTopicMutationRepository
{
    private readonly VocaNovaDbContext _dbContext;

    public PersonalTopicMutationRepository(VocaNovaDbContext dbContext)
    {
        _dbContext = dbContext;
    }

    public async Task<IReadOnlyCollection<PersonalTopic>> GetTopicsAsync(
        uint userId,
        uint? wordId = null,
        CancellationToken cancellationToken = default)
    {
        ...
        return topics.Select(topic =>
        {
            ...
            return ListMutationPersistenceMappings.ToPersonalTopic(
                topic.TopicId,
                list?.ListId,
                topic.TopicName,
                topic.TopicNameVi,
    ...
    public Task<bool> WordBelongsToTopicAsync(
        ...
        uint wordId,
        CancellationToken cancellationToken = default) =>
        _dbContext.WordTopics.AnyAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // CURRENT compatibility: this save remains separate from membership persistence.
