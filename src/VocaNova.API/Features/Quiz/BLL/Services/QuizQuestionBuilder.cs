...
public sealed class QuizQuestionBuilder : IQuizQuestionBuilder
{
    ...
    public async Task<QuizOperationResult<QuizQuestion>> BuildQuestionAsync(
        uint wordId,
        int questionType,
        CancellationToken cancellationToken = default)
    {
        if (wordId == 0)
        {
            return QuizOperationResult<QuizQuestion>.NotFound("Word not found.");
        }

        if (questionType is not WordToMeaning and not MeaningToWord and not Description)
        {
            return QuizOperationResult<QuizQuestion>.ValidationFailure("Question type is invalid.");
        }
        ...
        if (content is null)
        {
            return QuizOperationResult<QuizQuestion>.ValidationFailure("Word does not have enough quiz content.");
        }

        var candidates = await _quizQuestionRepository.GetDistractorsAsync(
            word.WordId,
            word.WordClass,
            word.TopicIds,
            cancellationToken);
        ...
        {
            return QuizOperationResult<QuizQuestion>.ValidationFailure(NotEnoughDistractorsMessage);
        }
        ...
        return QuizOperationResult<QuizQuestion>.Success(new QuizQuestion(
            word.WordId,
            word.SenseId,
            questionType,
    ...
    private static List<string> BuildDistractors(
        IReadOnlyCollection<QuizQuestionWord> candidates,
        int questionType,
    ...
    private static string? GetCandidateAnswer(QuizQuestionWord candidate, int questionType)
    {
