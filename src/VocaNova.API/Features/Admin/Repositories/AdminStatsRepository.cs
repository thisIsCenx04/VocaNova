...
public sealed class AdminStatsRepository : IAdminStatsRepository
{
    ...
    public async Task<AdminDemographicsDto> GetDemographicsAsync(CancellationToken cancellationToken = default)
    {
        var ageRanges = await (
                ...
                select new AdminDemographicGroupDto(
        ...
        return new AdminDemographicsDto(ageRanges, occupations, educationLevels);
    }

    public async Task<IReadOnlyCollection<AdminWrongWordDto>> GetTopWrongWordsAsync(
        int limit,
        CancellationToken cancellationToken = default)
    {
        ...
        return rows
            .Select(row => new AdminWrongWordDto(
                row.WordId,
                row.Word,
                row.WrongCount,
                row.CorrectCount,
