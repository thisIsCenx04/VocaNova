...
void main() {
  test('register sends backend contract and parses wrapped tokens', () async {
    ...
    final tokens = await AuthApiService(dio: dio).register(
      phone: '0901234567',
      password: 'Password1',
      displayName: 'Nhut',
      otpCode: '123456',
  ...
  test('register omits learning-profile fields that were left blank', () async {
    ...
    await AuthApiService(dio: dio).register(
      phone: '0901234567',
      password: 'Password1',
      displayName: 'Nhut',
      otpCode: '123456',
  ...
  test('parses the public learning-profile options catalog', () async {
    ...
    final options = await AuthApiService(dio: dio).getLearningProfileOptions();
  ...
  test('selectOnboardingTopics sends the topic id list', () async {
    ...
    final stored = await AuthApiService(
      dio: dio,
  ...
  test('login sends backend contract and parses wrapped tokens', () async {
    ...
    final tokens = await AuthApiService(
      dio: dio,
  ...
  test('sendOtp and verifyOtp use backend OTP contracts', () async {
    ...
    final repository = AuthApiService(dio: dio);
  ...
  test('getCurrentUser parses backend profile contract', () async {
    ...
    final user = await AuthApiService(dio: dio).getCurrentUser();
  ...
  test('updateLearningProfile sends nullable backend contract', () async {
    ...
    final user = await AuthApiService(dio: dio).updateLearningProfile(
  ...
  test(
    ...
    () async {
      ...
      final user = await AuthApiService(
        dio: dio,
  ...
  test('changePassword sends authenticated password contract', () async {
    ...
    final changed = await AuthApiService(dio: dio).changePassword(
  ...
  test('deleteAccount calls the authenticated account endpoint', () async {
    ...
    await AuthApiService(dio: dio).deleteAccount();
  });
}
