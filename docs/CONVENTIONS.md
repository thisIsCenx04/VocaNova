...
| BLL persistence ports | Production feature repository/cache/provider interfaces are logically BLL-owned through `Features.<Feature>.BLL.Abstractions` namespaces, but current files are physically under `Features/<Feature>/DAL/Repositories/Interfaces`; some test-only compatibility helpers still exist. | BLL-owned abstraction namespaces using BLL models/commands. |
